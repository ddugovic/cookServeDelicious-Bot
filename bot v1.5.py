...
def CitrusSteak():
    ...
    compare_imagesCooking()    #But ommit "number" variable so it doesen't click on the food that is cooking
    ...
    hitKey(str(cooking_numbers[0]))
    del cooking_timer[:] #this needs to be modified to delete the timer of the item I was cooking and not the whole list
...
#used to check if we are cooking more than 1 item
#if so we start poping the first [0] timer in new timer
cooking_counter = []
...
def compare_imagesCooking():
    """Almost identical to compare_images() function. But used solely for food that requires cooking.
       ...
       Key differences are it will ommit pressing the number of the customer it is already serving. i.e the food that is cooking.
    ...
    cooking_timer_new = list(cooking_timer)
    #cooking_timer_new.pop(cooking_timer_new[0])
    for i in cooking_numbers:
        numkeys_new.pop(i)
        numImages_new.pop(i)
    ...
    time.sleep(0.05)
    start = time.time()
    while time.time() < start + cooking_timer_new[]:
